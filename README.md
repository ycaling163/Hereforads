...
- **修改次数用完、买家仍然不通过**:待确认(见文末"还没拍板的问题")。
...
- 卖家违约时,管理员能不能强制从卖家的 Stripe 余额撤回钱(提议:只在违约这种情况下可以;余额不够就从下一笔收入里扣,同时暂停账号)
- 修改次数用完、买家仍然不通过初稿:提议买家可以取消订单,卖家保留 50% 作为 kill fee(创意行业常见的"中止费",补偿卖家已经投入的拍摄时间),另外 50% 退给买家;比例待定
- 英国消费者法:个人买家对服务类购买有 14 天取消权;提议结账时加一个勾选框"我要求立即开始服务,服务完成后放弃取消权"——要找律师确认
