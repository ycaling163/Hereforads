...
- **没开通 Stripe 也可以写个人主页、发布广告**(沿用 2026-09-19"发布免审核 + KYC 后置"的决定)。有人浏览卖家的广告页或给卖家发私信时,发邮件提醒卖家"开通 Stripe 才能收款"。这需要接入邮件服务(比如 [Resend](https://resend.com)),同一个卖家要限频(待确认:比如 3 天最多一封)。
- **没有通过平台收款的交易不享受平台担保**(私下交易风险自负,Terms 第 4 条已经有对应表述)。
...
- 卖家发布广告时自己填"交付天数",**默认 14 天**,从买家付款时开始算。
- **超期不会自动退款**:买家要自己发起"取消并退款",卖家同意才执行(卖家不回应或不同意,按第 5 条交给管理员)。

### 4. 取消与退款:必须选退款理由,理由决定手续费谁承担

每一次取消/退款都必须选一个理由,理由决定费用怎么分:

| 退款理由 | 发起方 | 退给买家 | 卖家要承担的费用 |
|---|---|---|---|
| 卖家在交付期限内拒单(不想接,比如内容低俗或跟自己的内容不匹配) | 卖家 | 全额 | **无**,Stripe 实际手续费由平台承担 |
| 超期没交付(包括生病等特殊原因,双方协商后取消) | 买家发起,卖家同意 | 全额 | **Cancellation fee = 退款金额对应的 Payment processing fee**,从卖家下一笔放款里扣 |
| 交付质量问题,双方协商 | 买家发起,填退款金额 | 全额或部分(退款时可以选具体金额) | 按比例:Service fee 和 Payment processing fee 按退款比例退还给卖家(见下) |
| 买家反悔,卖家同意(待确认) | 买家 | 全额 | 上轮提议由平台承担,还没有明确拍板 |

**部分退款按比例退手续费**:£100 订单退 £40 → 卖家少拿 40 − 4.80(Service fee 按比例退)− 1.68(Payment processing fee 按比例退)= £33.52,最终到手 £50.28,正好等于一笔 £60 订单的到手金额。

**取消次数约束**:90 天内取消 3 单,暂停卖家所有广告,管理员审核后恢复。**待确认**:"卖家在期限内拒单"算不算进这 3 单(拒单不收费,但如果算进来,可以防止卖家反复接单再拒)。

### 5. 裁决:谈不拢由管理员人工裁决

买家在 3 天确认期内申请退款时,自动放款立刻暂停;双方谈不拢(或卖家不回应,**待确认**超时天数,提议 3 天),订单进入管理员后台,管理员裁决全额退、部分退或驳回。**这一条改了 2026-09-18"平台不裁定履约质量"的立场**——平台现在会在双方谈不拢时做最终裁决,Terms 第 5 条("We don't arbitrate…")要跟着改。

### 6. 放款后的退款

- **只能在卖家同意的情况下退**,管理员不能强制(拒付除外,见第 7 条)。
- 流程(参考 Etsy"从卖家余额扣,不够再扣卖家银行账户"的做法):**先从卖家的 Connect 账户撤回转账(transfer reversal),撤回成功后才退给买家**,平台不垫钱。卖家 Stripe 余额不够(已经提现)的情况,打开 Stripe 的"负余额自动从卖家银行账户扣回"——这一点**必须先在测试模式下实际验证**各国是否可用;扣不回来就提示卖家余额不足,不执行退款。
- 卖家点"同意退款"时需要二次验证(重新输入密码或邮箱验证码),防止被盗号的人恶意退款。
- **退款时限(待确认)**:产品负责人的倾向是静态展示位 14 天(30 天太长);有播放量要求的广告需要等数据,时间要更长。参考:Collabstr 在交付后给买家 72 小时申请修改或纠纷,之后放款([Collabstr](https://collabstr.com/brand))。提议的简化方案是:**所有广告放款后退款时限统一 14 天**;有播放量要求的广告,发布时让卖家填一个"数据统计期"(比如 7 天,最长 30 天),**买家确认期从默认的 3 天延长到这个统计期**,钱在统计期内一直留在托管里,等数据出来再放款。这样放款后的规则只有一条,不用按广告类型分别处理。
...
- 放款后的退款时限,以及上面第 6 条提议的"播放量广告延长确认期"方案
- "卖家在期限内拒单"算不算进 90 天 3 单的次数
- 买家反悔、卖家同意时,Stripe 实际手续费谁承担
- 卖家不回应退款申请,多少天后转给管理员
- 5 个地区之外、还没开通 Stripe 的卖家的广告,买家能不能直接付款(现在的代码允许付款,但这类卖家开不了 Stripe、没法标记交付,钱会一直卡在托管里,最后只能退款),还是只显示"联系卖家"——后者等于开放私下交易的入口,跟 2026-09-18"平台责任边界"一节否决过的方案有冲突,需要明确决定
- 邮件提醒的限频规则
