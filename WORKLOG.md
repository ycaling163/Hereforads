...
- **线上发现两处跟记录不一致**:`social_accounts_url_http` 约束缺 `url = ''`(只填账号名的社交账号现在会保存失败,WORKLOG 之前记"已修正"但线上不是);`get_user_id_by_email` 还能被 anon/authenticated 调用(按邮箱查用户 ID)。README"模板化整理"里的那段 SQL **产品负责人已在线上执行并验证**(核对查询结果正确;只填账号名的社交账号能保存;已注册邮箱 guest 下单能到 Stripe 付款页)。
- PR #61 已合并。
...
## 2026-09-25 安全复查(模板化整理之后)

- 产品负责人要求复查:密钥、卖家互看、管理员可见性、其它漏洞。结论和细节见 README"安全复查"一节。
- 密钥:代码和全部 git 历史都没有;卖家之间隔离正常;管理员名单普通用户看不到。
- **发现并修了一个高危问题**:后台 7 个页面只靠 layout 检查管理员,未登录的人模拟站内跳转请求就能拿到页面内容(全部订单的买家联系方式、联系留言等)。本地复现后每个页面加 `requireAdmin()`,复测 9 个后台页面都只返回跳转登录。
- 中危:storage 的公开 select 策略让任何人能列出 bucket 文件(含私信图片)。产品负责人同意删,迁移已同步,**线上要执行** README 里的一条 SQL。
- 低危只记录没改:profiles/seller_profiles 的 Stripe 账户 ID 等列公开可读;删除文件因为没有 delete 策略实际没删。
