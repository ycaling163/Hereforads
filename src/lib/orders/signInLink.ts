 ...
 *
 * 返回 "rate_limited" 时让用户稍后再试;其他错误(包括邮箱没有账号)一律当成功处理,
 * 不让人借这个接口探测某个邮箱有没有注册。
 */
export async function sendSignInLink(email: string): Promise<"sent" | "rate_limited"> {
  const supabase = await createClient();
  const { error } = await supabase.auth.signInWithOtp({
    email,
    options: { shouldCreateUser: false, emailRedirectTo: `${SITE_URL}/auth/confirm` },
  });
  if (error) {
