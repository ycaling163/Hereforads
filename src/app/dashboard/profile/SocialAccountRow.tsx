    ...
    return (
      <li className="flex items-center justify-between gap-2 rounded-lg border border-zinc-200 px-4 py-2.5 text-sm">
        <span>
          ...
          {account.handle && (
            <span className="ml-2 text-zinc-500">{account.handle}</span>
          )}
          {account.url && (
            <a
              href={account.url}
              ...
              className="ml-2 text-zinc-500 underline decoration-zinc-300 underline-offset-2 hover:text-zinc-900"
            >
              {account.url}
            </a>
          )}
